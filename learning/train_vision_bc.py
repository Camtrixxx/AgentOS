...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a vision behavior cloning policy.")
    parser.add_argument("--data-dir", default="data/vision_demos")
    parser.add_argument("--output", default="checkpoints/vision_bc_policy.pt")
...
def main() -> None:
    ...
    if train_size <= 0:
        raise ValueError("Need at least two vision transitions for train/validation split")
...
def evaluate_loss(model: VisionBCPolicyNet, loader: DataLoader, action_weights: torch.Tensor) -> float:
    model.eval()
    total = 0.0
    count = 0
